    ...
    $.extend(
        {
            ...
            /**
             * 计算2日期之间的月数，用的是比较月份较的方法
             * 传进来的日期要么是Date类型，要么是yyyy-MM-dd格式的字符串日期
             * @param date1 日期一
             * @param date2 日期二
             */
            countMonths: function(date1, date2)
            {
                var fmt_month = 'MM';
                var fmt_year = 'yyyy';
                ...
                {
                    date1_year = this.formatDate(date1,fmt_year);
                    date1_month = this.formatDate(date1,fmt_month);
                    date2_year = this.formatDate(date2,fmt_year);
                    date2_month = this.formatDate(date2,fmt_month);
                }
                if(typeof date2_month === 'string' && typeof date2_month === 'string')
                {
                    var s = (date1_year - date2_year) * 12 + (date1_month - date2_month );
                    return s;
                }
                else
                {
