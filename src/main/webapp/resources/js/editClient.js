...
$(function(){
    ...
    $("#agentOwned").change(function () {
        var val = $(this).val();

        if(val == 0){
            layer.confirm('更改为下级客户使用后资质将重新认证，是否继续？')
        }else {
            layer.confirm('请确认代理商自用，否则将产生一定风险，是否继续？')
        }
    })
    ...
    $(".js-confirm").click(function(){
        ...
        if(!_Auth.isNull(params.mobile) && !_Auth.isMobile(params.mobile)){
            ...
            return;if(!_Auth.isNull(params.email) && !_Auth.isEmail(params.email)){
